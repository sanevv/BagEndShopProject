...
<script>
    ...
    $(function () {
        ...
        $('#verificationButton').on('click', function () {
            $.ajax({
                url: "${pageContext.request.contextPath}/api/mail/verify",
                type: "GET",
                data: {
                ...
                success: function (response) {
                    if(response){
                        window.location.href = "/test/resetPassword?email=${requestScope.email}";
                    }
                    else{
    ...
    function isEmailVerify() {
        return $.ajax({
            url: "${pageContext.request.contextPath}/api/mail/send",
            type: "POST",
...
<div class="container mt-5" style="max-width: 500px;">
    ...
    <form id="verifyForm">
        <div class="mb-3">
            ...
            <input type="radio" id="email" name="authMethod" value="email" checked/>
            <label for="email">이메일</label>
        </div>

        <div class="mb-3">
            <label class="form-label fw-bold mb-2">이메일</label>
            <span id="userEmail">${requestScope.email}</span>
