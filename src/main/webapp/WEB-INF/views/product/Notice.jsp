...
function goEdit(el) {
    if (confirm("수정할꺼야?")) {
        const noticeID = $(el).data("id");
        location.href = "${pageContext.request.contextPath}/notice/update?noticeID=" + noticeID;
    }
}

function goDelete(el) {
    if (confirm("삭제해?")) {
        const noticeID = $(el).data("id");
...
<div class="text-center" id="notice_head">
    ...
    <div class="dropdown custom-dropdown">
        <button class="btn btn-light btn-sm dropdown-toggle border-0" type="button"
            id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false" title="관리 메뉴">
            <i class="bi bi-gear-fill"></i>
        </button>
        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
            <li><a class="dropdown-item" href="#" data-id="${nvo.notice_id}" onclick="goEdit(this)">수정하기</a></li>
            <li><a class="dropdown-item text-danger" href="#" data-id="${nvo.notice_id}" onclick="goDelete(this)">삭제하기</a></li>
        </ul>
    </div>

    <!-- NOTICE 제목 -->
    <span style="color: #7ff000; font-size: 20px; font-weight: bold; line-height: 100px;">NOTICE</span>
	
    <h2 class="title" style="font-weight: bold; font-size: 20pt;">${nvo.title}</h2>
	
    <p id="date" style="font-size: 10pt; margin: 30px 0 100px 0; color: rgba(0, 0, 0, 0.5);">
        작성일 <span style="color: rgba(0, 0, 0, 0.7);">${nvo.created_at}</span>
    </p>

	<img alt="" src="${nvo.thumnail}">
